...
|------|------|------|-----------|-------------|
| 막힘 | 비어 있음 | 무관 | 좌회전 | 장애물 감지→정지 ≤50ms; 결정→모터 명령 ≤500ms |
| 막힘 | 막힘 | 비어 있음 | 우회전 | 장애물 감지→정지 ≤50ms; 결정→모터 명령 ≤500ms |
| 막힘 | 막힘 | 막힘 | 후진 → 좌·우 재조회 → 좌/우 회전 | 정지→후진 ≤100ms; 후진 후 결정 ≤500ms; 결정→모터 명령 ≤500ms |
...
| UC5 Avoid Obstacle – Turn Right | UC-03 장애물 회피 (경로 2) | 방향 결정 표 두 번째 행으로 흡수 |
| UC6 Avoid Obstacle – Move Backward & Turn | UC-03 장애물 회피 (경로 3) | 방향 결정 표 세 번째 행으로 흡수 |
...
**통합 근거 (UC3-6 → UC-03)**: 원본의 UC3은 장애물 감지 후 좌·우 센서를 조회하고 UC4/5/6으로 분기하는 "결정 허브"이다. UC4·5·6은 각각 단일 분기 경로에 불과하며 독립적 트리거가 없다. 세 분기를 방향 결정 표로 표현하면 한 유스케이스로 충분히 기술되고 검증 시그널도 더 명확해진다.
