...
>
> **유지보수 노트 (TP#6)**: `legacy/docs/specs/requirements.md`를 인계받아 **오른쪽 근접 센서 삭제(센서 3→2)** 만 반영. 변경/일치는 문서 끝에 표기.
...
전방 장애물이 감지되면 시스템은 전진 이동을 중지하고 좌측 센서 상태를 조회하여 아래 결정 표에 따라 이동 방향을 전환해야 한다.
...
> <sup>※</sup> 후진 후 우회전은 우측 근접 센서 없이 수행되는 최후 탈출(blind)이다. 삭제된 것은 우측 **센서**(입력)이며, 우회전 **모터 명령**(출력)은 존속한다.

**검증 기준**:
- 전방 감지 → 이동 완전 정지까지 ≤ 50ms
- 좌회전 결정 후 모터 명령 발생까지 ≤ 500ms
- 전방·좌 모두 막힌 경우: 정지 후 ≤ 100ms 내 후진 시작; 후진 완료 후 ≤ 500ms 내 방향 결정; 결정 후 ≤ 500ms 내 모터 명령

#### FR-MOVE-03 — 회전 중 입력 마스크
회전 동작(좌회전 또는 우회전)이 진행되는 동안에는 전방·좌측 근접 센서 및 먼지 센서로부터 유입되는 모든 신호를 무시하고 회전을 완수해야 한다. 회전 완료 후 정상 센서 폴링을 재개한다.
...
- 강화 청소 모드 중에도 전방·좌측 근접 센서 입력은 정상적으로 처리하여 FR-MOVE-02 장애물 회피를 실행한다.
...
구동 모터, 청소 장치, 각 센서(전방·좌측 근접·먼지) 는 순수 추상 C++ 클래스(인터페이스)로 정의되어야 한다. 애플리케이션 로직은 이 인터페이스만 참조하며 구체 구현에 직접 의존하지 않는다.
...
| NFR-TIMING-04a | 전·좌 모두 막힘 → 후진 시작 | ≤ 100ms | FR-MOVE-02 후진 경로 진입 |
...
| R3.4 | ~~FR-MOVE-02 (경로 2: 우회전)~~ | **(TP#6) 제거** — 우측 근접 센서 삭제로 "우측 비면 우회전" 사전 조회 경로 폐지 |
| R3.5 | FR-MOVE-02 (경로 2: 후진+회전) | 통합 (방향 결정 표의 두 번째 행) |
...
| 항목 | legacy 대비 | 내용 |
|---|---|---|
| FR-MOVE-02 제목·요구사항 | 변경 | "3-경로 → 2-경로", "좌·우측 센서 조회 → 좌측 센서 조회" |
| FR-MOVE-02 방향 결정 표 | 변경 | 우측 열·"우회전" 단독 행 삭제; 후진 행을 좌측 재조회 기반으로 단순화(최후 탈출 우회전 주석) |
| FR-MOVE-02 검증 기준 | 변경 | "전방·좌·우 모두 막힘" → "전방·좌 모두 막힘" |
| FR-MOVE-03 | 변경 | "전방·좌·우 근접 센서" → "전방·좌측 근접 센서" |
| FR-CLEAN-02 부속 규칙 | 변경 | "전방·좌·우 근접 센서" → "전방·좌측 근접 센서" |
| NFR-ARCH-01 | 변경 | "각 센서(전방·좌·우·먼지)" → "(전방·좌측 근접·먼지)" |
| NFR-TIMING-04a | 변경 | "전·좌·우 모두 막힘" → "전·좌 모두 막힘" |
| 추적성 표 R3.4 | 변경 | "경로 2: 우회전" 제거(우측 센서 삭제); R3.5를 "경로 2"로 재번호 |
| FR-CTRL-*, FR-MOVE-01, FR-CLEAN-01, FR-SENSE-*, 그 외 NFR | 일치 | 센서 무관, legacy 그대로 |
