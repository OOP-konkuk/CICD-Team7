# Class Diagram (OOD) — RVC Control SW

> **Phase**: OOD in the 1st Elaboration  
> **Input**: `01-inception/requirements.mdx` + `02-ooa/domain-model.mdx` + `02-ooa/ssd.mdx`  
> **Output**: Design Class Diagram — 가시성, 타입, 메서드 시그니처를 추가한 설계 클래스 다이어그램

---

## Domain Model → Design Class 변환

| 개념 클래스 (OOA) | 설계 클래스 (OOD) | 추가된 설계 사항 | 도출 근거 |
|-----------------|-----------------|----------------|-----------|
| `RVCOrchestrator` | `RVCOrchestrator` | System Operations(`onTick`, `onFrontDetected`, `onDustDetected`)를 public으로, `avoidFrontObstacle` / `avoidAllObstacles`를 private으로 정의 | SSD System Operations + UC-2 Alt |
| `Sensor` (abstract) | `Sensor` (abstract) | `# detected: bool`, `+ detect(): bool` | Domain Model + NFR-3 확장성 |
| `FrontSensor` / `LeftSensor` / `RightSensor` / `DustSensor` | 동일 | `+ detect(): bool` 구체 구현 | Domain Model |
| `MotorController` | `MotorController` | `setDirection()` / `getDirection()` 시그니처 확정 | Domain Model |
| `CleanerController` | `CleanerController` | `setMode()` / `getMode()` 시그니처 확정 | Domain Model |
| `DigitalClock` | `DigitalClock` | `tick(): void` | Domain Model |
| `Direction` enum | `Direction` | FORWARD / BACKWARD / LEFT / RIGHT | Domain Model + I/O Events |
| `CleanMode` enum | `CleanMode` | ON / OFF / UP | Domain Model + I/O Events |

---

## PlantUML Code

```plantuml
@startuml ClassDiagram_OOD
title RVC Control SW — Design Class Diagram (OOD)

enum Direction {
  FORWARD
  BACKWARD
  LEFT
  RIGHT
}

enum CleanMode {
  ON
  OFF
  UP
}

abstract class Sensor {
  # detected : bool
  + detect() : bool
}

class FrontSensor {
  + detect() : bool
}

class LeftSensor {
  + detect() : bool
}

class RightSensor {
  + detect() : bool
}

class DustSensor {
  + detect() : bool
}

class DigitalClock {
  + tick() : void
}

class MotorController {
  - direction : Direction
  + setDirection(dir : Direction) : void
  + getDirection() : Direction
}

class CleanerController {
  - mode : CleanMode
  + setMode(mode : CleanMode) : void
  + getMode() : CleanMode
}

class RVCOrchestrator {
  + onTick() : void
  + onFrontDetected() : void
  + onDustDetected(val : bool) : void
  - avoidFrontObstacle() : void
  - avoidAllObstacles() : void
}

Sensor <|-- FrontSensor
Sensor <|-- LeftSensor
Sensor <|-- RightSensor
Sensor <|-- DustSensor

RVCOrchestrator "1" --> "1" FrontSensor        : reads >
RVCOrchestrator "1" --> "1" LeftSensor         : reads >
RVCOrchestrator "1" --> "1" RightSensor        : reads >
RVCOrchestrator "1" --> "1" DustSensor         : reads >
RVCOrchestrator "1" --> "1" DigitalClock       : listens >
RVCOrchestrator "1" --> "1" MotorController    : controls >
RVCOrchestrator "1" --> "1" CleanerController  : controls >

MotorController   ..> Direction  : uses
CleanerController ..> CleanMode  : uses
@enduml
```

---

## Diagram

![Class Diagram OOD](./images/ClassDiagram_OOD.png)

---

## 관계 설명

| 관계 | From | To | 도출 근거 |
|------|------|----|-----------|
| Generalization | FrontSensor / LeftSensor / RightSensor / DustSensor | Sensor | Domain Model + NFR-3 확장성 |
| Association (reads) | RVCOrchestrator | FrontSensor / LeftSensor / RightSensor / DustSensor | SD-1~3: detect() 직접 호출 |
| Association (listens) | RVCOrchestrator | DigitalClock | UC-1 Actor: Digital Clock |
| Association (controls) | RVCOrchestrator | MotorController | Domain Model — 방향 제어 위임 |
| Association (controls) | RVCOrchestrator | CleanerController | Domain Model — 청소 제어 위임 |
| Dependency (uses) | MotorController | Direction | Domain Model |
| Dependency (uses) | CleanerController | CleanMode | Domain Model |
