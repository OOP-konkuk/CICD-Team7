...
**문서 버전**: 1.1 (TP#6 유지보수)  
**작성일**: 2026-05-21 (초판) / **개정일**: 2026-05-29 (오른쪽 근접 센서 삭제 반영)  
**상태**: 유지보수 개정 (Maintenance Revision)  
**입력 문서**: `docs/srs/SRS.md` (단일 진실 소스, TP#6 개정본)

> **유지보수 노트 (TP#6)**: 본 SDD는 `legacy/docs/sdd/SDD.md`(v1.0)를 기준선으로 인계받아 **오른쪽 근접 센서 삭제(센서 3→2)** 가 강제하는 부분만 최소 수정한 것이다. 클래스 구조·이름·계층·파일 분할은 legacy 그대로 보존한다. 변경/일치 내역은 문서 끝 [변경 이력](#변경-이력-tp6-유지보수)에 명시한다.
...
| `FrontSensor`, `LeftSensor` (각각 별도) | `IProximitySensor` (단일 인터페이스, 2 인스턴스) | 센서의 계약이 동일하므로 인터페이스를 통합; 인스턴스를 위치(FRONT/LEFT) 역할로 구분 **(TP#6: 우측 센서 삭제로 3→2 인스턴스)** |
...
**의존하는 추상화**: `IProximitySensor` (2개), `IDustSensor`, `ICommandSource`, `IOutputLog`  
...
**책임**: 전방 장애물 감지 후 2-경로 방향 결정(FR-MOVE-02), 회전 원자 실행(FR-MOVE-03 — 회전 중 모든 센서 입력 무시), 전진 이동 제어(FR-MOVE-01).

**의존하는 추상화**: `IProximitySensor` (2개), `IDriveTrain`, `IOutputLog`
...
private:
    AvoidPath selectPath() const;                    // 좌·후진 경로 결정
    void      executeTurn(MotorCommand turnCmd);     // FR-MOVE-03: 회전 원자 실행
    void      executeBackAndTurn();                  // 2번째 경로: 후진 후 재조회
...
> **유지보수 주(註)**: legacy SDD는 본 ctor를 `(front, left, right, drive, log)`로 기술했고 본 개정에서 `right`를 제거했다. 한편 legacy **구현 코드**(`legacy/include/core/obstacle_handler.hpp`)는 `front` 없이 `(left, right, drive, log)`였다 — 즉 `front` 파라미터는 **TP#6 이전부터 존재한 SDD↔코드 불일치**다. 본 단계는 우측 센서 삭제 범위만 다루므로 이 `front` 불일치는 수정하지 않고 인지만 기록한다. `/code` 단계는 빌드 진실인 구현 코드를 기준으로 `right`만 제거한다.
...
**후진 후 회전 방향 결정**: `executeBackAndTurn()`은 후진 완료 후 **좌측 센서만** 재조회하여, 좌측이 비면 좌회전, 좌측이 여전히 막혀 있으면 우회전(우측 근접 센서 없이 수행되는 최후 탈출)을 실행한다. 우측 센서를 사용하지 않으므로 우측 센서 삭제는 이 메서드의 로직을 바꾸지 않는다.
...
> **유지보수 일치(註)**: 본 인터페이스는 방향(전방/좌측/우측)을 명시하지 않는 **방향 비특정** 계약(`isBlocked()`)이다. 우측 센서 삭제는 인터페이스가 아니라 그 **인스턴스 개수**(3→2)에만 영향을 주므로, `IProximitySensor` 자체는 legacy와 **일치**한다.
...
**명칭 근거**: PDF의 `moveForward()`, `turnLeft()`, `moveBackward()` 등 개별 메서드 대신 단일 `execute(MotorCommand)` 메서드로 통합하여 ISP를 유지하고 새 명령 추가 시 인터페이스 변경 없이 `MotorCommand` 열거자만 확장한다. (우회전을 포함한 모든 이동 명령은 본 인터페이스의 `MotorCommand`로 그대로 표현된다 — 우측 **센서** 삭제와 무관.)
...
sequenceDiagram
    ...
    note over OH : selectPath() 호출 → 전·좌 모두 막힘 판정
    OH  ->> LOG : write("전방·좌 막힘 — 후진 후 재탐색")
    OH  ->> DT  : execute(BACKWARD)      %% ≤100ms (NFR-TIMING-04a)
    note over OH : executeBackAndTurn() 내부 후진 완료 대기
    OH  ->> LS  : isBlocked()            %% 후진 완료 후 좌측만 재조회 (NFR-TIMING-04b)
    OH  ->> DT  : execute(TURN_LEFT 또는 TURN_RIGHT)   %% 좌측 비면 좌회전 / 막힘이면 우회전(blind), ≤500ms (NFR-TIMING-04c)
    note over OH : executeTurn() 내부: 센서 미호출 (FR-MOVE-03)
    OH  ->> DT  : execute(FORWARD)
```
...
enum class MotorCommand {
    ...
    BACKWARD,
    TURN_LEFT,
    TURN_RIGHT,    // 모터 구동 명령(우회전) — 우측 *센서* 삭제와 무관하게 존속
    STOP
};
...
enum class AvoidPath {
    TURN_LEFT,        // 좌측 공간 있음
    BACK_AND_TURN     // 전·좌 모두 막힘 → 후진 후 좌측 재조회
};
...
> **유지보수 주(註)**: `AvoidPath`에서 `TURN_RIGHT` 경로를 삭제했다. 이는 "좌측 막힘·우측 비어 있음 → 우회전"이라는 **우측 센서 판독에 의존한 경로 선택지**였으므로 우측 센서 삭제와 함께 폐지된다. 반면 `MotorCommand::TURN_RIGHT`(모터 구동 명령)는 후진 후 최후 탈출 등에서 여전히 발행되므로 **존속**한다.
...
> **유지보수 일치(註)**: 파일 분할·디렉토리 구조는 legacy와 **완전히 일치**한다. 우측 센서 삭제는 새 파일을 추가하거나 기존 파일을 삭제하지 않으며, 일부 파일의 내용만 수정한다(센서 인스턴스/배선/회피 분기). 따라서 본 매핑 표는 legacy 그대로다.
...
```bash
# WSL Ubuntu 환경
wsl bash -lc "cd '/mnt/c/.../#5_Team7_NoRight' && \
...
TEST(ObstacleHandlerTest, LeftTurnWhenLeftIsClear) {
    ...
    ObstacleHandler handler(front, left, drive, log);
...
> **유지보수 주(註)**: 위 예시는 legacy SDD의 `(front, left, right, …)`에서 `right`만 제거한 형태다(§3.2의 `front` 불일치 註 참조). 실제 단위 테스트의 정확한 픽스처·시그니처는 `/ut` 단계가 빌드 진실인 구현 코드를 기준으로 확정한다.
...
| NFR-TIMING-04b | `ObstacleHandler` | 후진 후 좌측 재조회 — 500ms 이내 |
...
> 기준선: `legacy/docs/sdd/SDD.md` (v1.0). 변경 명세: `reference/new_objective.md` (오른쪽 근접 센서 삭제). 상위 입력: `docs/srs/SRS.md` v1.1.

| 컴포넌트/다이어그램/시그니처 | legacy 대비 | 내용 |
|---|---|---|
| 문서 헤더 | 변경 | 버전 1.0→1.1, 개정일·유지보수 노트 추가 |
| §2.3 분해 차이 표 (센서 행) | 변경 | "Front/Left/RightSensor → 3 인스턴스(FRONT/LEFT/RIGHT)" → "Front/LeftSensor → 2 인스턴스(FRONT/LEFT)" |
| §3.1 CleaningEngine 의존 추상화 | 변경 | "IProximitySensor (3개)" → "(2개)" |
| §3.1 CleaningEngine ctor | 변경 | `IProximitySensor& rightSensor` 인자 삭제 |
| §3.1 CleaningEngine 멤버 | 변경 | `IProximitySensor& right_;` 삭제 |
| §3.2 ObstacleHandler 책임 | 변경 | "3-경로 방향 결정" → "2-경로 방향 결정" |
| §3.2 ObstacleHandler 의존 추상화 | 변경 | "IProximitySensor (3개)" → "(2개)" |
| §3.2 ObstacleHandler ctor | 변경 | `IProximitySensor& right` 인자 삭제 (`front` 인자는 §3.2 註의 선재 불일치로 미변경) |
| §3.2 ObstacleHandler 멤버 | 변경 | `IProximitySensor& right_;` 삭제 |
| §3.2 selectPath 주석 | 변경 | "좌·우·후진 경로 결정" → "좌·후진 경로 결정" |
| §3.2 후진+회전 설명 | 추가 | 후진 후 좌측만 재조회 → 좌회전/우회전(blind) 설명 명시 |
| §4.1 IProximitySensor 인터페이스 | **일치** | 방향 비특정(`isBlocked()`) — 인스턴스 수만 변동, 인터페이스 불변 (일치 註 추가) |
| §4.3 IDriveTrain | 일치 | `execute(MotorCommand)` 불변; 우회전은 모터 명령으로 존속 (명칭 근거에 한 줄 보강) |
| §6.2 좌회전 시퀀스 | 일치 | front·left만 등장, 우측 센서 무관 → legacy 그대로 |
| §6.3 후진+재조회 시퀀스 | 변경 | `participant RS(right)` 삭제; "전·좌·우 막힘" → "전·좌 막힘"; `OH->>RS:isBlocked()` 삭제; 회전 라인 주석을 좌측 재조회 기반으로 갱신 |
| §6.1·6.4·6.5 시퀀스 | 일치 | 우측 센서 무관 |
| §7 MotorCommand::TURN_RIGHT | **일치** | 모터 구동 명령으로 존속 (주석 보강) |
| §7 AvoidPath::TURN_RIGHT | 변경 | enum 멤버 삭제 (우측 센서 의존 경로 선택지) |
| §7 AvoidPath::BACK_AND_TURN 주석 | 변경 | "전·좌·우 모두 막힘" → "전·좌 모두 막힘" |
| §8 디렉토리·파일 매핑 | **일치** | 파일 추가·삭제 없음 (일치 註 추가) |
| §10.4 단위 테스트 예시 | 변경 | `StubProximitySensor front, left;` / `handler(front, left, drive, log)` (`right`만 제거) |
| §11 추적성 매트릭스 | 일치 | FR/NFR ID·담당 컴포넌트 변동 없음 (NFR-TIMING-04b 문구만 "좌측 재조회"로 미세 보강) |
| §1·§2.1·§2.2·§5·§9 등 | 일치 | 계층 구조·상태 모델·빌드 설계 등 센서 무관 부분 legacy 그대로 |

**선재(先在) 불일치 플래그**: legacy SDD §3.2/§10.4의 `ObstacleHandler` ctor는 `front`를 포함하나 legacy 구현 코드는 `front` 없이 `(left, right, drive, log)`다. 이는 우측 센서 삭제와 무관한 **TP#6 이전부터의 SDD↔코드 불일치**이므로 본 단계에서 수정하지 않고 §3.2 註로 기록만 한다. `/code`는 빌드 진실인 구현 코드를 기준으로 `right`만 제거한다.

---

## 자기 점검 결과 (TP#6 유지보수)

- [x] legacy의 클래스·인터페이스·계층·디렉토리 구조를 보존했다(재설계 없음). 변경은 우측 센서 의존성·회피 분기·시퀀스에 국한.
- [x] 우측 센서 의존성·시그니처·시퀀스 분기를 일관되게 제거했다(CleaningEngine·ObstacleHandler ctor·멤버, §6.3 시퀀스, AvoidPath).
- [x] 방향 비특정 추상화(`IProximitySensor`)·파일 매핑·`MotorCommand::TURN_RIGHT`를 "일치"로 명시했다.
- [x] 시그니처를 C++ 코드블록으로 정확히 적어 `/code`가 받아쓸 수 있다(선재 `front` 불일치는 註로 플래그).
- [x] PDF·`CICD-Team7-main/`·`objective.md`를 본 단계에서 열지 않았다.
