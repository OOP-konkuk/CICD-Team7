...
> **유지보수 노트 (TP#6)**: 본 SRS는 `legacy/docs/srs/SRS.md`(TP#5 v1.0)를 기준선으로 인계받아 **오른쪽 근접 센서 삭제(센서 3개 → 2개)** 가 강제하는 부분만 최소 수정한 것이다. 변경/일치 내역은 문서 끝 [8. 변경 이력](#8-변경-이력-tp6-유지보수)에 명시한다. 그 외 모든 요구사항은 legacy와 **일치**한다.
...
RVC 제어 소프트웨어는 물리적 환경과 사용자 명령을 입력으로 받아 로봇의 이동·청소 동작을 제어하는 임베디드 제어 SW이다. 시스템은 3종의 센서에서 실시간 이벤트를 받고, 3종의 물리 장치에 명령을 내린다.
...
|------|------|-----------|
| 막힘 | 비어 있음 | 좌회전 → 전진 재개 |
| 막힘 | 막힘 | 후진(일정 거리) → 좌측 재조회 → 좌회전(좌측 비면) 또는 우회전<sup>※</sup>(좌측 막힘이면) → 전진 재개 |

> <sup>※</sup> 후진 후 좌측이 여전히 막혀 있을 때의 우회전은 **센서 판독 없이 수행되는 최후 탈출(blind last-resort)** 이다. 우측 근접 센서가 제거되었으므로 우측 공간을 사전 조회하지 않으며, 후진으로 확보된 여유를 이용한 탈출 시도로만 의미를 갖는다.
...
|------|------|
| **ID** | FR-MOVE-03 |
| **요구사항** | 회전 동작(좌회전 또는 우회전)이 진행되는 동안에는 전방·좌측 근접 센서 및 먼지 센서로부터 유입되는 모든 신호를 무시하고 회전을 완수해야 한다. 회전 완료 후 정상 센서 폴링을 재개한다. |
...
| 장애물 회피 허용 | 강화 모드 중에도 전방·좌측 근접 센서 입력은 정상적으로 처리하여 FR-MOVE-02 장애물 회피를 실행한다. |
...
|------|------|
| **ID** | NFR-ARCH-01 |
| **요구사항** | 구동 모터, 청소 장치, 각 센서(전방·좌측 근접·먼지)는 순수 추상 C++ 클래스(인터페이스)로 정의되어야 한다. 애플리케이션 로직은 이 인터페이스만 참조하며 구체 구현에 직접 의존하지 않는다. |
...
| 막힘 | 막힘 | 후진 → 좌측 재조회 → 좌회전(좌측 비면) 또는 우회전<sup>※</sup>(좌측 막힘이면) | 정지→후진 ≤100ms; 후진 후 결정 ≤500ms; 결정→모터 ≤500ms |

> <sup>※</sup> FR-MOVE-02의 주석과 동일: 후진 후 우회전은 우측 센서 판독 없이 수행되는 최후 탈출이다.
...
물리 장치(모터, 청소 장치, 센서 3종)와의 통신은 모두 순수 추상 인터페이스를 통해 이루어진다 (NFR-ARCH-01). 구체 인터페이스 이름, 메서드 시그니처, 헤더 구조는 SDD에서 결정한다.
...
> **주**: 구동 모터의 출력 명령에는 좌회전·**우회전**이 모두 남는다. 우회전은 모터 구동 명령이며, 삭제된 것은 우측 **근접 센서**(입력)이지 우회전 **이동 능력**(출력)이 아니다.
...
> **유지보수 주**: FR/NFR/UC의 ID·집합은 legacy와 **일치**한다(삭제·추가된 요구사항 없음). 우측 센서 삭제는 FR-MOVE-02·UC-03의 내용을 단순화했을 뿐 추적 관계를 바꾸지 않았다.
...
> 기준선: `legacy/docs/srs/SRS.md` (v1.0). 변경 명세: `reference/new_objective.md` (오른쪽 근접 센서 삭제).

| 섹션/항목 | legacy 대비 | 내용 |
|---|---|---|
| 문서 헤더 | 변경 | 버전 1.0 → 1.1, 개정일·유지보수 노트 추가 |
| §2 도입문 ("4종의 센서") | 변경 | "4종 → 3종의 센서" (우측 근접 센서 제거) |
| §2.1 컨텍스트 다이어그램 | 변경 | `RS["우측 근접 센서"]` 노드·엣지 삭제 |
| §2.2 외부 액터 표 | 변경 | "우측 근접 센서" 행 삭제 |
| §2.3 운영 모드 | 일치 | M-AVOID 설명이 방향 비특정이라 그대로 |
| §3.2 FR-MOVE-02 제목 | 변경 | "(3-경로 결정)" → "(2-경로 결정)" |
| §3.2 FR-MOVE-02 요구사항 | 변경 | "좌·우측 센서 조회" → "좌측 센서 조회" |
| §3.2 방향 결정 표 | 변경 | 우측 열 삭제, "우회전" 단독 행 삭제, 후진 행을 좌측 재조회 기반으로 단순화 + 최후 탈출 주석(※) |
| §3.2 측정 기준 표 | 변경 | "전·좌·우 모두 막힘" → "전·좌 모두 막힘"; "좌/우 회전 결정" → "좌회전 결정" |
| §3.2 FR-MOVE-03 | 변경 | "전방·좌·우 근접 센서" → "전방·좌측 근접 센서" |
| §3.3 FR-CLEAN-02 부속 규칙 | 변경 | "전방·좌·우 근접 센서" → "전방·좌측 근접 센서" |
| §4.1 NFR-ARCH-01 | 변경 | "각 센서(전방·좌·우·먼지)" → "각 센서(전방·좌측 근접·먼지)" |
| §4.2 NFR-TIMING-04a | 변경 | "전·좌·우 모두 막힘 → 후진" → "전·좌 모두 막힘 → 후진" |
| §4.2 NFR-TIMING-03 | 일치 | 행 표현은 legacy 유지 (좌/우 회전 결정 → 모터; 우회전은 모터 명령으로 존속) |
| §5 UC-03 주 액터 | 변경 | "전방·좌측·우측 근접 센서" → "전방·좌측 근접 센서" |
| §5 UC-03 주요 흐름·결정 표 | 변경 | "좌·우 센서 조회" → "좌측 센서 조회", 우측 열·우회전 행 삭제 |
| §6.2 하드웨어 추상화 경계 | 변경 | "센서 4종 → 3종"; "우측 근접 센서 인터페이스" 행 삭제; 모터 명령에 우회전 존속(주 추가) |
| §3.1·3.3·3.4 기타 FR | 일치 | FR-CTRL-*, FR-MOVE-01, FR-CLEAN-01, FR-SENSE-* 등 센서 무관 요구사항 legacy 그대로 |
| §4.3·4.4·4.5 NFR | 일치 | NFR-BUILD/UI/SAFETY legacy 그대로 |
| §7 추적성 매트릭스 | 일치 | FR/NFR/UC ID·관계 변동 없음 (요구사항 삭제·추가 없음) |
| §1.4 참고 문서 | 일치 | 워크플로우 문구는 우측 센서와 무관 → 최소 수정 원칙에 따라 미변경 |

**핵심 의미 변화**: 회피 경로 선택이 "좌 가능→좌회전 / 좌 막힘→후진"으로 단순화(우측 센서 사전 조회 경로 제거). 단, 후진 후 좌측이 막혀 있을 때의 우회전은 우측 센서 없이 수행되는 최후 탈출로 **존속**(코드 `executeBackAndTurn`은 좌 센서만 사용 → 변경 불요).

---

## 자기 점검 결과 (TP#6 유지보수)

- [x] legacy SRS를 출발점으로 삼고, 오른쪽 센서와 무관한 요구사항을 재작성하지 않았다.
- [x] 센서 개수(3→2)·회피 로직 단순화가 FR-MOVE-02·FR-MOVE-03·FR-SENSE·UC-03·§2·§6에 일관 반영되었다.
- [x] "센서(입력) 삭제"와 "우회전(모터 출력) 존속"을 구분하여, 우회전 이동 능력을 잘못 삭제하지 않았다.
- [x] 변경/일치 표가 SRS의 모든 영향 항목을 덮는다 (§8).
