...
ObstacleHandler::ObstacleHandler(
    ...
    IDriveTrain&      drive,
    IOutputLog&       log)
    : left_(left), drive_(drive), log_(log)
{}
...
void ObstacleHandler::avoidAndReturn() {
    ...
    case AvoidPath::BACK_AND_TURN:
        log_.write("[AVOID] 전방·좌 막힘 — 후진 후 재탐색");
        executeBackAndTurn();
        break;
    }
...
void ObstacleHandler::executeBackAndTurn() {
    ...
    // Re-check the left sensor after backing up (NFR-TIMING-04b).
    // If left is clear, turn left; otherwise turn right as a blind
    // last-resort escape (the right proximity sensor was removed, so the
    // right side is not polled — only the backed-up clearance is used).
