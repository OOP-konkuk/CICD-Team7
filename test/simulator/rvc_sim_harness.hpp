...
class RVCSimHarness {
public:
    RVCSimHarness() : engine_(front_, left_, right_, dust_, drive_, cleaner_, log_, cmds_) {}

    // -----------------------------------------------------------------------
    // Environment injection API — call before run()
    // -----------------------------------------------------------------------

    // Set steady-state front/left/right sensor values
    void setFrontBlocked(bool b)  { front_.setBlocked(b); }
    void setLeftBlocked(bool b)   { left_.setBlocked(b); }
    void setRightBlocked(bool b)  { right_.setBlocked(b); }
    ...
    void injectRightObstacle(bool blocked)  { right_.enqueueEvent(blocked); }
    ...
    SimProximitySensor& rightSensor()  { return right_; }
...
private:
    SimProximitySensor front_, left_, right_;
    SimDustSensor      dust_;
