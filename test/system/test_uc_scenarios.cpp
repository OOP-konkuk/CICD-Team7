...
// Path 2 — left blocked, right clear: TURN_RIGHT chosen
TEST(UC03_ObstacleAvoidance, RightPath_WhenLeftBlockedRightClear) {
    RVCSimHarness h;
    h.injectFrontObstacle(true);
    h.setFrontBlocked(false);
    h.setLeftBlocked(true);
    h.setRightBlocked(false);
    ...
    EXPECT_TRUE(h.driveHasCommand(MotorCommand::TURN_RIGHT));
    EXPECT_TRUE(h.logContains("우회전"));
}

// Path 3 — all blocked: BACKWARD then turn
TEST(UC03_ObstacleAvoidance, BackPath_WhenAllBlocked) {
    RVCSimHarness h;
    h.injectFrontObstacle(true);
    h.setFrontBlocked(false);
    h.setLeftBlocked(true);
    h.setRightBlocked(true);
...
TEST(UC06_ErrorShutdown, SensorException_TriggersErrorHaltWithLog) {
    ...
    CleaningEngine engine(faulty, left, right, dust, drive, cleaner, log, cmds);
    engine.run();
